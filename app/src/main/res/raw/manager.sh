...
direct_install_system(){
    ...
    if [ "$(grep_cmdline androidboot.hardware)" == "titan" ]; then
        echo "- VphoneGaga kernel is detected"
        vphonegaga_titan=true
    fi 
   ...
   if mountpoint -q /vendor; then
    ...
    if [ -d "/sys/fs/selinux" ]; then
        if ! $vphonegaga_titan && ! "$INSTALLDIR/magiskpolicy" --live "permissive su" &>/dev/null; then
